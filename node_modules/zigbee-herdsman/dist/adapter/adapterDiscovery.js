...
function matchUsbFingerprint(portInfo, entries, isWindows, conflictProne) {
    ...
    if (match) {
        if (score > 1 /* UsbFingerprintMatchScore.VID_PID */) {
            if (conflictProne && score < 3 /* UsbFingerprintMatchScore.VID_PID_PATH */ && matchString(match.manufacturer, 'itead')) {
...
async function findUsbAdapterBestMatch(adapter, portInfo, isWindows, conflictProne) {
    let bestMatch;
    ...
    try {
        ...
        for (const portInfo of portList) {
            ...
            const bestMatch = isWindows
                ? undefined
                : await findUsbAdapterBestMatch(undefined, portInfo, isWindows, USB_FINGERPRINTS_CONFLICT_IDS.includes(`${portInfo.vendorId}:${portInfo.productId}`));
